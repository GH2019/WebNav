...
const UIController = {
        ...
        Object.keys(DataStore.cards).forEach(cardType => {
            ...
            cardGroupData.forEach((businessGroup, businessGroupIndex) => {
                ...
                businessGroup.items.forEach((cardItem, cardIndex) => {
                    ...
                    // 将内容添加到卡片 - 调整各个元素添加到卡片的顺序
                    card.appendChild(cardLogo);
                    card.appendChild(cardContent);
                    
                    // 将删除按钮添加到卡片 - 最后添加确保在最上层
                    card.appendChild(deleteButton);
                    
                    // 将编辑按钮添加到卡片 - 最后添加确保在最上层
                    card.appendChild(editButton);
                    
                    /* 气泡提示功能已禁用 - 如需启用请取消注释
                    // 准备工具提示内容
                    const tooltipContent = this.createTooltipContent(cardItem.title, cardItem.description);
                    
                    // 为整个卡片内容区域添加鼠标悬停事件
                    cardContent.addEventListener('mouseenter', () => {
                        TooltipHandler.show(cardContent, tooltipContent);
                    });
     ...
     */
    bindEvents: function() {
        // 导航项点击事件已经在renderNavigation方法中处理，这里不需要重复添加
    ...
    handleNavItemClick: function(event) {
        ...
        if (targetGroup) {
            ...
            // 获取目标元素的位置
            const targetPosition = targetGroup.getBoundingClientRect().top + window.pageYOffset;
            
            // 计算考虑了头部高度的滚动位置
            const scrollPosition = targetPosition - headerHeight - 20; // 额外减去20px的边距
    ...
    highlightCardGroup: function(cardGroup) {
        ...
        document.querySelectorAll('.card-group').forEach(group => {
            group.style.borderBottom = '1px solid rgba(246, 138, 78, 0.05)';
        });
        
        // 为当前卡片组添加高亮边框
        cardGroup.style.borderBottom = '2px solid var(--primary-color)';
        // 延迟移除高亮，恢复正常状态
        setTimeout(() => {
            cardGroup.style.borderBottom = '1px solid rgba(246, 138, 78, 0.05)';
        }, 2000);
    },
        ...
        setTimeout(() => {
            ...
            if (!foundResults) {
                ...
                if (firstVisibleGroup) {
                    ...
                    // 获取目标元素的位置
                    const targetPosition = firstVisibleGroup.getBoundingClientRect().top + window.pageYOffset;
